...
select chefs.name, 
group_concat(distinct restaurants.name separator ', ') as RestName, -- finds where each chef works at and use distinct as restraurants shows up twice
avg(foods.price) as AvgPrice -- Calculate the average price of food each chef makes
from works
inner join chefs on works.chefID = chefs.chefID
inner join restaurants on works.restID = restaurants.restID  -- join chefs to restaurants throught works table
inner join serves on serves.restID = restaurants.restID  -- join restaurants table with serves table
